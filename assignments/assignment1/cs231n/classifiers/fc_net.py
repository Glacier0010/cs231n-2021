...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dx, grads['W2'], grads['b2'] = affine_backward(ds, cache)
        dx, grads['W1'], grads['b1'] = affine_relu_backward(dx, relu_cache)
        
        loss += 0.5 * self.reg * np.sum(W1 * W1)    #regularization
        loss += 0.5 * self.reg * np.sum(W2 * W2)    #do not need to *2 in the last 2 lines
